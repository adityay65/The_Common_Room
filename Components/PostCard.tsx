...
export default function PostCard({ post }: { post: Post }) {
  ...
  return (
    ...
    <Link href={`/posts/${post.id}`} className="block"> 
      <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300 transform hover:-translate-y-1 h-full">
        ...
        <div className="p-6 flex flex-col">
          ...
          <h3 className="text-xl font-bold text-gray-900 mb-2 truncate">{post.title}</h3>
            {/* Added min-h-[5rem] for consistent card height */}
          <p className="text-gray-600 text-base mb-4 min-h-[5rem] overflow-hidden">{post.content}</p>
